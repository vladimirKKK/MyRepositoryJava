...
public class PresentationMembers {
    ...
    public static void findFileInCatalog(String dir, List<String> filesFound,
        ...
        /* формат вывода даты создания файла */
        ...
        if (listFiles != null) {
            ...
            for (int i = 0; i < listFiles.length; i++) {
                ...
                if (listFiles[i].isDirectory()) {
                    ...
                    /*
                     * если найден каталог - выполнить метод printCatalog() с
                     * новым путем
                     */
                    findFileInCatalog(direct, filesFound, date);
                } else {
                    /*
                     * если файл - проверить на соответствие шаблону и добавить
                     * в список
                     */
                    Matcher m = p.matcher(direct);
                    if (m.matches()) {
                        /* получение даты создания в заданном формате */
                        String dateMakeFile = df.format(new java.util.Date(
                                listFiles[i].lastModified()));
                        /* сравтить даты создания */
                        if (dateMakeFile.endsWith(date))
                            /*
                             * дата создания соответствует - поместить имя файла
                             * в список
                             */
            ...
            /* если в исходной директории нет файлов */
        } else
                        ...
                        if (participantFullInfo
                            ...
                            participantFullInfo
                                    ...
                                    .get(participantFullInfo.get(idExit)
                                            .getTimeInOut().size() - 1)
                                    .setOutputTime(t);
                            /* если поле OutputTime not null - записать значение */
                        } else {
        ...
        if (args.length == 4) {
            ...
            System.out
                    .println("Введите исходные данные в формате: каталог для поиска, "
                            + "название лекции,  имя каталога выходных файлов, содержащих "
                            + "информацию о участниках и дату проведения (в формате dd.mm.yyyy)");
    }
    ...
    private static void writeTo(OutputStream os,
        ...
        for (Attendence entry : participantFullInfo) {
            ...
            for (SaveTime saveTime : entry.getTimeInOut()) {
                ...
                /* если дата null - преобразовывать из даты в строку не надо */
                ...
                /* построение результирующей строки по заданному формату */
